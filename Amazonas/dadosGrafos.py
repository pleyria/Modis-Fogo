...
for a in range (2003, 2020):
	...
	plt.savefig(rd + "/GraficosAno/" + ano + "/mean_degree", dpi = 600)
	plt.close()
	
	# plot betweenness
	plt.plot(m, mean_betweenness)
	plt.title("mean betweenness " + ano)
	plt.savefig(rd + "/GraficosAno/" + ano + "/mean_betweenness", dpi = 600)
	plt.close()
	
	# plot closeness
	plt.plot(m, mean_closeness)
	plt.title("mean closeness " + ano)
	plt.savefig(rd + "/GraficosAno/" + ano + "/mean_closenness", dpi = 600)
	plt.close()
	
	# plot entropy
	plt.plot(m, entropy)
	plt.title("entropy " + ano)
	plt.savefig(rd + "/GraficosAno/" + ano + "/entropy", dpi = 600)
	plt.close()
