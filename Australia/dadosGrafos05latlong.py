...
f = os.listdir(rd + "/LatLong05/grafosMes") # lista com os nomes dos grafos
...
dadosGrafos.to_csv(rd + "/LatLong05/csv/dadosGrafosMensais.csv")
...
for a in range (2003, 2020):
	...
	plt.savefig(rd + "/LatLong05/GraficosAno/" + ano + "/mean_degree", dpi = 300)
	plt.close()
	
	# plot betweenness
	plt.plot(m, mean_betweenness)
	plt.title("mean betweenness " + ano)
	plt.savefig(rd + "/LatLong05/GraficosAno/" + ano + "/mean_betweenness", dpi = 300)
	plt.close()
	
	# plot closeness
	plt.plot(m, mean_closeness)
	plt.title("mean closeness " + ano)
	plt.savefig(rd + "/LatLong05/GraficosAno/" + ano + "/mean_closenness", dpi = 300)
	plt.close()
	
	# plot entropy
	plt.plot(m, entropy)
	plt.title("entropy " + ano)
	plt.savefig(rd + "/LatLong05/GraficosAno/" + ano + "/entropy", dpi = 300)
	plt.close()
